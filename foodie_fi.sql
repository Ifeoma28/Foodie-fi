...
plan_name VARCHAR(13), price DECIMAL(5,2));
...
customer_id INT,plan_id INT, start_date DATE);
...
-- how many customers foodie-fi has ever had
SELECT COUNT(DISTINCT customer_id) AS total_customers
FROM subscriptions;

-- monthly distribution of trial plan start_date values using start of the month as group by
...
SELECT COUNT( DISTINCT customer_id)   AS customer_count,plan_name FROM
...
-- what is the customer count and percentage of customers who have churned rounded to 1 d.p
...
-- one table for free trials and another for their next plan where the date difference is 8 days
...
WITH next_plan AS (
SELECT customer_id,start_date, ROW_NUMBER() OVER(PARTITION BY s.customer_id ORDER BY start_date) AS rnk,plan_name
FROM subscriptions s
INNER JOIN plans p ON s.plan_id = p.plan_id 
)
...
-- what is the customer count and percentage breakdown of all 5 plans values at 2020-12-31
...
WITH last_plan AS (
	...
	SELECT COUNT(DISTINCT customer_id) AS customer_count,plan_name,((CAST (COUNT(plan_name) AS FLOAT)) /1000)*100 AS customer_plans_percent
...
SELECT COUNT ( *) AS annual_upgrades FROM
	(SELECT DISTINCT s.customer_id,s.plan_id,p.plan_name,s.start_date
	FROM subscriptions s
	INNER JOIN plans p ON
	s.plan_id = p.plan_id
...
-- 195 customers upgraded to annual plans
 ...
 -- how many days on average does it take for a customer to an annual plan from the day they join foodie_fi
...
WITH free_trial_users AS (
    ...
    -- converting the average number of days to reach annual once they join foodie-fi into 30-day periods
    -- 104/30 to give us 3.47times
